...
| 测试编号 | 测试场景 | 测试内容 | 验证点 |
|---------|---------|---------|--------|
| TC-001 | FileNode 属性验证 | 设置/获取所有属性 | 属性值正确存储 |
| TC-002 | FileNode 父子关系 | 添加子节点、设置父节点 | 双向引用正确 |
| TC-003 | FileNode 路径计算 | 获取完整路径 | 路径拼接正确 |
| TC-004 | FileDetails 元数据 | 设置 EPUB 元数据 | 所有字段可读写 |
| TC-005 | ParseResult 状态 | 成功/失败状态切换 | 状态一致性 |
| TC-006 | NodeType 枚举 | 所有枚举值定义 | 枚举值正确 |

**Services 层测试**

| 测试编号 | 测试场景 | 输入 | 预期输出 | 验证点 |
|---------|---------|------|---------|--------|
| TC-007 | 标准 EPUB 2.0 解析 | 有效 EPUB 2.0 文件 | ParseResult.Success = true | 元数据、文件树完整 |
| TC-008 | 标准 EPUB 3.0 解析 | 有效 EPUB 3.0 文件 | ParseResult.Success = true | 支持 NAV 导航 |
| TC-009 | EPUB 2.0 + NCX 目录 | 旧版 EPUB | 正确解析 NCX | 目录结构完整 |
| TC-010 | 损坏 ZIP 处理 | 损坏的 EPUB 文件 | ParseResult.Success = false | 错误信息明确 |
| TC-011 | 非 EPUB 文件 | .txt 文件 | 返回 null 或错误 | CanParse 返回 false |
| TC-012 | 空文件 | 0 字节文件 | 返回错误 | 边界处理正确 |
| TC-013 | 大文件解析 | >100MB EPUB | 解析成功，内存占用合理 | 懒加载生效 |
| TC-014 | 异步解析取消 | 大文件 + CancellationToken | 操作取消，资源释放 | 无内存泄漏 |
| TC-015 | 并发解析 | 同时解析 5 个文件 | 全部成功 | 线程安全 |
| TC-016 | 解析器工厂 | 注册/获取解析器 | 正确分发 | 工厂模式正确 |

**Utilities 层测试**

| 测试编号 | 测试场景 | 测试内容 | 验证点 |
|---------|---------|---------|--------|
| TC-017 | MIME 类型识别 | 各种文件扩展名 | 识别正确 |
| TC-018 | 路径处理 | 相对/绝对路径转换 | 路径正确 |
| TC-019 | 文件大小格式化 | 各种字节数 | 格式化正确 |
| TC-020 | 校验和计算 | 文件内容 | 计算正确 |

**IContentLoader 测试**

| 测试编号 | 测试场景 | 输入 | 预期输出 | 验证点 |
|---------|---------|------|---------|--------|
| TC-021 | 懒加载文本 | 文本节点 | 文本内容 | 按需加载 |
| TC-022 | 懒加载二进制 | 图片节点 | 字节数组 | 内容正确 |
| TC-023 | 缓存命中 | 重复访问同一节点 | 从缓存读取 | 性能优化 |
| TC-024 | 缓存失效 | 清除缓存后访问 | 重新加载 | 缓存管理正确 |
| TC-025 | 加载不存在的条目 | 无效路径 | 抛出异常 | 错误处理正确 |

#### 2.6.3 集成测试用例

| 测试编号 | 测试场景 | 输入 | 预期输出 | 验证点 |
|---------|---------|------|---------|--------|
| TC-026 | 完整解析流程 | EPUB 文件路径 | ParseResult | 端到端正确 |
| TC-027 | 解析后遍历树 | 解析结果 | 遍历所有节点 | 树结构完整 |
| TC-028 | 解析后加载内容 | 解析结果 + 节点选择 | 内容正确 | 联动正确 |
...
| 测试编号 | 测试场景 | 输入 | 预期输出 | 验证点 |
|---------|---------|------|---------|--------|
| TC-043 | PreviewService 初始化 | 创建服务实例 | 服务就绪 | 依赖注入正确 |
| TC-044 | HTML 预览生成 | HTML 节点 | PreviewViewModel | 类型为 Html |
| TC-045 | 图片预览生成 | 图片节点 | PreviewViewModel | 类型为 Image |
| TC-046 | 文本预览生成 | CSS 节点 | PreviewViewModel | 类型为 Text |
| TC-047 | 二进制预览生成 | 未知类型节点 | PreviewViewModel | 类型为 Binary |
| TC-048 | 预览内容加载 | 大图片 | 缩略图 | 内存优化 |
| TC-049 | 预览缓存 | 重复预览 | 从缓存读取 | 性能优化 |

#### 4.4.2 交互功能测试

| 测试编号 | 测试场景 | 操作 | 预期结果 | 验证点 |
|---------|---------|------|---------|--------|
| TC-050 | HTML 预览 | 选择 HTML 节点 | 右栏渲染网页 | 样式图片正确 |
| TC-051 | 图片预览 | 选择图片节点 | 显示图片 | 缩放适应 |
| TC-052 | 代码高亮 | 选择 CSS 节点 | 语法高亮显示 | 关键字着色 |
| TC-053 | 拖拽打开 | 拖拽 EPUB 到窗口 | 文件加载成功 | 反馈明确 |
| TC-054 | 文件关联 | 双击 EPUB 文件 | 应用启动并加载 | 参数传递正确 |
| TC-055 | 错误处理 | 打开损坏文件 | 显示错误提示 | 不崩溃 |
| TC-056 | 加载状态 | 打开大文件 | 显示进度指示 | 防止假死 |
| TC-057 | WebView2 资源拦截 | HTML 中的图片 | 从 ZIP 加载 | 资源路径正确 |
| TC-058 | 多文件拖拽 | 拖拽多个文件 | 加载第一个 | 处理正确 |
...
| 测试编号 | 测试场景 | 测试内容 | 验证点 |
|---------|---------|---------|--------|
| TC-059 | BaseParser 异常处理 | 模拟异常 | 错误信息正确 |
| TC-060 | BaseParser 节点创建 | 调用 CreateNode | 节点属性正确 |
| TC-061 | PlainTextParser CanParse | .txt 文件 | 返回 true |
| TC-062 | PlainTextParser CanParse | 非 .txt 文件 | 返回 false |
| TC-063 | PlainTextParser Parse | 有效 txt | ParseResult.Success |
| TC-064 | PlainTextParser Parse | 无效路径 | 错误处理正确 |
| TC-065 | 工厂注册新解析器 | 注册 PlainTextParser | 工厂包含新解析器 |
| TC-066 | 工厂获取解析器 | .txt 文件 | 返回 PlainTextParser |

#### 5.5.2 性能与扩展性测试

| 测试编号 | 测试场景 | 操作 | 预期结果 | 验证点 |
|---------|---------|------|---------|--------|
| TC-067 | 扩展性验证 | 加载 .txt 文件 | 正确显示结构 | 新解析器生效 |
| TC-068 | 内存泄漏 | 连续打开 100 个大文件 | 内存稳定 | 无泄漏 |
| TC-069 | 并发性能 | 同时解析多个文件 | 完成时间合理 | 线程安全 |
| TC-070 | 缓存性能 | 重复访问相同资源 | 响应时间 < 10ms | 缓存命中 |
| TC-071 | 大文件性能 | 解析 500MB EPUB | 时间 < 30s | 性能达标 |
| TC-072 | 安装包测试 | 安装 MSI | 正常安装运行 | 注册表正确 |
| TC-073 | 便携版测试 | 解压 ZIP 运行 | 无需安装 | 依赖完整 |
| TC-074 | 升级测试 | 安装新版本 | 覆盖安装成功 | 数据保留 |
